...
function GroceryListItem({ checklistItem, listQuantity, groceryListData, setGroceryListData }) {   
  const [quantityChecker, setQuantityChecker] = useState(listQuantity(checklistItem.product_name)); 
  ...
  return (
    <div className="checklist-container">
      ...
      <div className="checklist-item-info">
        ...
        <form className="checklist-quantity-form">
          <input type="text" name="checklist-quantity-input" value={quantityChecker} />
        </form>
        <p className="checklist-item-price">${Math.round(price*100*quantityChecker)/100}</p>
      </div>
    </div>
